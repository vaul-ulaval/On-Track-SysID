from on_track_sys_id.helpers.vehicle_dynamics_stown import vehicle_dynamics_st
...
class Simulator:
    ...
    def run_simulation(
        self, initialState, u, duration=SIMULATION_DURATION, dt=SIMULATION_DT
    ):
...
class LookupGenerator:
    ...
    def load_lookup(self, model, name):
        ...
        file_path = os.path.join(
            package_path, "models", model, name + "_lookup_table.csv"
        )
    ...
    def generate_lookup(self):
        fine_steers = np.linspace(
            START_STEER,
            STEER_FINE_END,
            int((STEER_FINE_END - START_STEER) / FINE_STEP_SIZE),
            endpoint=False,
        )
        coarse_steers = np.linspace(
            STEER_FINE_END,
            END_STEER,
            int((END_STEER - STEER_FINE_END) / COARSE_STEP_SIZE),
        )
        steers = np.concatenate((fine_steers, coarse_steers))
        vels = np.linspace(
            START_VEL, END_VEL, int((END_VEL - START_VEL) / VEL_STEP_SIZE)
        )
    ...
    def plot_lookup(self):
        ...
        ax.plot(
            [vel, vel], [steer, steer], [0, a_lat], c="r", linestyle="--", zorder=10
        )
        ax.plot(
            [vel, vel], [START_STEER, steer], [0, 0], c="r", linestyle="--", zorder=10
        )
        ax.plot(
            [START_VEL, vel], [steer, steer], [0, 0], c="r", linestyle="--", zorder=10
        )
        plt.show()
    ...
    def save_lookup(self):
        ...
        file_path = os.path.join(
            ...
            self.save_LUT_name + "_lookup_table.csv",
        )
