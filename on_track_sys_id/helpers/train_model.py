...
from on_track_sys_id.helpers.generate_predictions import generate_predictions
from on_track_sys_id.helpers.generate_inputs_errors import generate_inputs_errors
from on_track_sys_id.helpers.SimpleNN import SimpleNN
from on_track_sys_id.helpers.pacejka_formula import pacejka_formula
from on_track_sys_id.helpers.plot_results import plot_results
from on_track_sys_id.helpers.solve_pacejka import solve_pacejka
from on_track_sys_id.helpers.save_model import save
from on_track_sys_id.helpers.load_model import get_dotdict
from on_track_sys_id.helpers.simulate_model import LookupGenerator
...
def get_model_param(racecar_version):
    ...
    yaml_file = os.path.join(
        package_path, "models", racecar_version, racecar_version + "_pacejka.txt"
    )
    with open(yaml_file, "r") as file:
...
def generate_training_set(training_data, model):
    ...
    X_train, y_train = generate_inputs_errors(
        v_y_next_pred, omega_next_pred, training_data
    )
...
def nn_train(training_data, racecar_version, save_LUT_name, plot_model):
    ...
    for i in range(1, num_of_iterations + 1):
        if (
            i == num_of_iterations
        ):  # Determine if it's the last iteration to enable plotting (if plot_model is False)
            plot_model = True
        ...
        pbar = tqdm(
            total=num_of_epochs,
            desc=f"Iteration: {i}/{num_of_iterations}, Epoch:",
            ascii=True,
        )
        ...
        for epoch in range(1, num_of_epochs + 1):
            ...
            train_loss = criterion(
                outputs, y_train
            )  # + nn_model.l2_regularization_loss() # TODO add regularization if needed
            optimizer.zero_grad()
            train_loss.backward()
            optimizer.step()
            ...
            if epoch == num_of_epochs:
                ...
                C_Pf_identified, C_Pr_identified = solve_pacejka(
                    model, v_x, v_y, omega, delta
                )
